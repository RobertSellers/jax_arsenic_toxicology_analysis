...
prepare_data_for_stan_test <- function(ft, dat, log_ = FALSE, scale_ = TRUE){
...
pred_ed_long_all_join$selection <- NA
pred_ed_long_all_join$selection[pred_ed_long_all_join$individual %in% c(unique(resilient_mito$individual),unique(resilient_hoescht$individual))
] <- "high_mean"
  pred_ed_long_all_join$selection[pred_ed_long_all_join$individual %in% c(unique(sensitive_mito$individual),unique(sensitive_hoescht$individual))
] <- "low_mean"                              

pred_ed_long_all_join$selection <- pred_ed_long_all_join$selection
# sensitive_features <- subset(pred_ed_long_all_join, individual %in% c(sensitive_hoescht$individual, sensitive_mito$individual))

selected_features <- subset(pred_ed_long_all_join, individual %in% c( resilient_hoescht$individual, resilient_mito$individual,sensitive_hoescht$individual, sensitive_mito$individual))

selected_features <- subset(selected_features,!is.na(selection))
pred_ed_long_all_join$selection <- as.factor(pred_ed_long_all_join$selection)
pred_ed_long_all_join$group <- "hoescht"
pred_ed_long_all_join$group[pred_ed_long_all_join$feature %in% c("mito_edge","mito_intensity","mito_nucleinumberofobjects","mito_area")] <- "mito"

# pred_ed_long_all_join$color_1 <- 
# 
# pred_ed_long_all_join$color_2 <-
```

```{r}
# high_low_features <- function(dataset, criteria){
#   
# }
# random_features <- function(dataset, criteria){
#   
# }
...
ggplot(pred_ed_long_all_join) +
   geom_point(mapping = aes(x = ed50_dose, y = pred_response, 
                          color = feature), size = 0.4) + 
    scale_color_brewer(type = 'div', palette = 7, direction = 1, na.value = NA) +
    ggalt::geom_encircle(aes(x = ed50_dose, y = pred_response, group = individual, color = selection),  s_shape = 1.5, expand = 0, show.legend = FALSE) + theme_dark() +
