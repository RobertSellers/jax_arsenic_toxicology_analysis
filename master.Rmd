...
__<span style="color:red; font-size:26px;">`r paste("-> ",sub('./', '', selected_dir))`</span>__ was selected in this file's code.
...
plate_collection_list <- harmony_read_plate_2(dir = "./do_focus")
...
- Creates an amalgamated data.frame from _harmony_read_plate's_ returned list object's constituent plate data and metadata.

```{r}
all_plates_df <- do.call(rbind.fill, lapply(plate_collection_list, function(x) {x@plate_df})) %>%
  tidyr::replace_na(list(concentration = 999)) %>% # change NA to 999 for ease of qa
  mutate(concentration = factor(concentration))

#any(is.na(all_plates_df$concentration))
#which(all_plates_df$concentration==999)
#all_plates_df[all_plates_df$concentration==999, ]  
#write.csv(all_plates_df[all_plates_df$concentration == 999, ], "~/Desktop/temp/missing_do.csv")  
# contains appended headers from each PlateResults.txt
plate_metadata_df <- do.call(rbind.fill, lapply(plate_collection_list, function(x) {x@plate_metadata}))
```

# Customize data

- From this point forward things are not very automatic
...
doscreen_sample_lookup <- readr::read_csv("lookups/do_unfiltered/sample_lookup.csv") %>% 
  clean_names() %>%
  rowwise() %>% 
...
# all_plates_df_join<- all_plates_df_join %>% 
...
__`r length(unique(all_plates_df_join$individual))`__ individual DO mice were identified for __`r length(all_plates_df_join$individual)`__ total wells. 2 technical replicates per individual per plate. Biological replicates on alternative A or B plates.
...
DT::datatable(schema_construction, rownames=FALSE, caption="Concentration frequencies by sex and experiment (plate)")
```

- __Total individuals in harmony output__ `r length(unique(all_plates_df_join$individual))`

- __Total plates__   `r length(unique(all_plates_df_join$run))` 

- __Total wells__ `r nrow(all_plates_df_join)`

- __Missing concentration__ `r percent(nrow(all_plates_df_join[all_plates_df_join$concentration == 999, ])/nrow(all_plates_df))`

- __Missing plate__ `r percent(1- mean(complete.cases(all_plates_df_join$run)))`

- __Missing sex__ `r percent(1- mean(complete.cases(all_plates_df_join$sex)))`


```{r}
schema_construction_wells <- all_plates_df_join %>%
...
DT::datatable(schema_construction_wells, rownames=FALSE, caption="Individual concentration frequencies")
```
