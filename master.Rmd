...
founders_quantal_collection <- harmony_create_collection(dir = "./founders_quantal")
do_focus_collection <- harmony_create_collection(dir = "./do_focus")
```
...
randomize_pca <- function(df,rand.individual,rand.feature, dose.val){
  ...
  na.omit %>%
    filter(
      individual %in% sample(levels(individual), rand.individual)
    )
  ...
  df_pca <- suppressWarnings(
    prcomp(random_n_plates[,sort(sample(c(start_idx:end_idx),rand.feature))], 
           scale=TRUE))
  
p<- autoplot(df_pca, 
         main = paste0("Dose:",as.character(dose.val)," with ",rand.individual," individuals and ",rand.feature, " features"),
         data = random_n_plates, 
         colour = "run", 
         label=TRUE,
         label.size = 2,
         loadings = TRUE, 
...
return (list(p,random_n_plates[,sort(sample(c(start_idx:end_idx),rand.feature))]))
}
```

```{r}
# removing columns for st_dev
minus_stdev <- do_focus_collection$all_plates[, -grep("std_dev", colnames(do_focus_collection$all_plates))]
minus_stdev_mean <- do_focus_collection$all_plates[, -grep("mean", colnames(do_focus_collection$all_plates))]
```

```{r}
#set.seed(11)
# random concentration
random_concentration <- sample(do_focus_collection$all_plates$concentration, 1)


val <- randomize_pca(
  do_focus_collection$all_plates,
  rand.individual = 3, # num random individuals
  rand.feature= 5, # num random features
  dose.val = random_concentration
  )
val[[1]]
# ok <- cbind(stringr::str_split_fixed(rownames(val[[2]]), " ",2),val[[2]])
# ok  %>%
#   ggplot( aes(x=in_focus_intensity_nucleus_alexa_488_median_std_dev, color=`1`, fill=`1`)) +
