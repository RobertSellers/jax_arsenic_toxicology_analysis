...
- Aggregate dataset with curated mitotracker / mitochondria-related features
  1. nuclei_numberofobjects
  2. infocus_intensity_cell_mitotrackerdeepred_mean_mean
  3. infocus_cell_mitotrackerdeepred_ser_edge_1px_mean
  4. infocus_cell_area_mm2_mean
...
# f_ <- formula(pheno ~ fdose  + (1 | group_pair/dir) + (1 | plate)) # robert's formula
f_ <- formula(pheno ~ (1|mouse) + (1|fdose) + (1|group_pair/dir)) # gary's formula
# 'log_num_nuclei','log_mito_uniformity','log_mito_fluor_intensity','mito_area'
##################################################
data$check_log_num <- batch_correct(formula_ = f_, 'log_num_nuclei', data, r.eff = 'dir:group_pair',target_col = 'dir')
# data$check_log_num <- batch_correct(formula_ = f_, 'check_log_num', data, r.eff = 'group_pair',target_col = 'group_pair')
##################################################
data$check_log_mito_uniformity <- batch_correct(formula_ = f_, 'log_mito_uniformity', data, r.eff = 'dir:group_pair',target_col = 'dir')
...
##################################################
data$check_log_mito_area <- batch_correct(formula_ = f_, 'log_mito_area', data, r.eff = 'dir:group_pair',target_col = 'dir')
# data$check_mito_area <- batch_correct(formula_ = f_, 'check_mito_area', data, r.eff = 'group_pair',target_col = 'group_pair')
##################################################
data$check_log_mito_fluor_intensity <- batch_correct(formula_ = f_, 'log_mito_fluor_intensity', data, r.eff = 'dir:group_pair',target_col = 'dir')
...
set.seed(1)
subset_n <- data_adj %>% group_by(individual) %>% sample_n_groups(40)
...
require(medrc)
  ...
  data_adj_for_medrc <- data_adj_for_medrc %>% mutate(mean_check_log_mito_area = ifelse(is.na(mean_check_log_mito_area), min(mean_check_log_mito_area, na.rm = T), mean_check_log_mito_area))

  #curve specific difference to the parameter average
  check_plate <- medrc::metadrm(
    mean_check_log_num ~ dose,
    data = na.omit(data_adj_for_medrc),
    fct = LL.4(),
    ind = dir,
    struct = "UN"
  )
  ...
  rownames(rmat) <- levels(data$dir)
  colnames(rmat) <- letters[2:5]
    
 m <-medrm(
    mean_check_log_mito_area ~ dose,
    data = data_adj_for_medrc,
    fct = LL.4(),
    curveid = b + c + d+e  ~ findividual,
    # random effects
    random = b+c+d+e~ 1| dir/mouse/fdose,
    # weights = varExp(form = ~ mean_check_log_num),
