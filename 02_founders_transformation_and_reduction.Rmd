...
feature_reduction_lplot <- function(mouse_metrics, results){
  ...
  sd_df_all <- extract_mean_stdev(mouse_metrics$mean_stdev_df_list,
          n = nrow(mouse_metrics$mean_stdev_df_list$interval_0_0.1))
  
  sd_df_all$sd_rank <- ifelse(rownames(sd_df_all) %in% c(results$keep$name,results$remove$name), TRUE, FALSE)
  
  sd_df_all_melt <- reshape2::melt(as.matrix(sd_df_all)) %>%
    dplyr::rename(feature = Var1, stdev = Var2)
  
  sd_df_all_melt$sd_rank <- ifelse(sd_df_all_melt$feature %in% c(results$keep$name,results$remove$name), TRUE, FALSE)
  
  ss_sd_df_all_melt <-sd_df_all_melt[!(sd_df_all_melt$stdev=="sd_rank"),]
  
  ss_sd_df_all_melt$sd_rank <- as.character(as.logical(ss_sd_df_all_melt$sd_rank))
  
  ss_sd_df_all_melt$sd_rank <- ifelse(ss_sd_df_all_melt$feature %notin% rownames(results$keep), ss_sd_df_all_melt$sd_rank, 2)
  
  ss_sd_df_all_melt<- ss_sd_df_all_melt %>%
    mutate(sd_rank = ifelse(feature %in% results$keep$name, "good", sd_rank))
  
  ss_sd_df_all_melt$sd_rank <- plyr::mapvalues(ss_sd_df_all_melt$sd_rank,
            from=c("TRUE","FALSE","good"),
            to=c("b","a","c"))
  
  ggplot(arrange(ss_sd_df_all_melt[order(ss_sd_df_all_melt$sd_rank),])) +
    labs(title=paste0("Feature StDev across Dose Response\n","corr value threshold < ",results$parameters$corr_threshold,", n = ",results$parameters$target_n),
    ...
    scale_color_manual(values=c('#DAA520','#00CED1','#FF5733'))+
    #scale_alpha_discrete(range = c(0.2, 1, 0.2)) +
...
results <- suppressWarnings(filter_exceedances(list_data = mouse_metrics, # input list object
                                               ...
                                               corr_cut_off = 0.20, # makes underlying values positive
...
feature_reduction_lplot(mouse_metrics = mouse_metrics, results = results)
```
