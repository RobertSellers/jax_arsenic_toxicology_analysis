...
for (i in 1:nrow(sub_stdev)){
  ...
  print(paste("removing", length(remove),"/",n,"features"))
  return (remove)
}
removals <- filter_exceedances(list_data = test, n = 50, corr_cut_off = 0.9)
```

```{r}
removals <- filter_exceedances(list_data = test, n = 13, corr_cut_off = 0.7)
```

```{r}
n <- 30
vals <- feature_exceedance_extraction(dr_list_object = test, corr_cut_off = 0.90, n = n)
sub_stdev <- extract_mean_stdev(test$stdev_df_list, n=n)
sub_stdev$name <- rownames(sub_stdev)
vals$name <- gsub("\\..*","",rownames(vals))
...
png("output/mean_dr_correlation_heatmap.png", width = 4000, height = 4000)
...
```{r}
# find high overall correlations

for (i in 1:nrow(corr_matrix_mean_all)){
  correlations <-  which((corr_matrix_mean_all[i,] > 0.85) & (corr_matrix_mean_all[i,] != 1))
  
  if(length(correlations)> 0){
    print(colnames(corr_matrix_mean_all)[i])
    print(correlations)
  }
}
```
