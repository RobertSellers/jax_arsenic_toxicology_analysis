...
prepare_ed50s_stan <- function(model_, original_data){
  ...
  df <- df %>% 
    ...
    # calculates the mean across columns in a matrix, based on ((chains-warmup)*iterations)
...
```{r}
f_ <- formula(pheno ~ (1|mouse) + (1|fdose) + (1|dir)) 
...
stan_prep_hoescht_dark$response  <- batch_correct(formula_ = f_, 'response', stan_prep_hoescht_dark, r.eff = 'dir', target_col = 'dir', debug=FALSE)
...
stan_prep_hoescht_dark_avgs_ <- stan_prep_hoescht_dark %>%
  group_by(fdose, dose, individual_id) %>%
  summarise_at(vars(response), funs(mean(., na.rm=TRUE)))
...
indv_slope_estimate<- stan_prep_hoescht_dark %>% 
...
- fit the population dose-response curve.  Take the averaged response across 4x replicates at each dose for each mouse (1 value per mouse per dose) and fit a single curve.  This will yield one ED50 for the measured outcome, plus confidence and prediction intervals.  This is a useful way to determine if a trait is going to give good DR behavior and to get a ball park (prior) for the individual fits.
- individual fits.  Reduce data to just one mouse.  That is 4x reps at each dose.  Fit the DR curve, estimate the ED50.  Use flat priors. Repeat for all mice. This step will give us per-mouse estimates of ED50 but they will be unstable due to small number of reps.
- fit curve to each mouse but using informative priors.  Two ways to do this – hierarchical model in Stan.  Could be a computational issue, but best solution.  Alternative is repeat individual fits but use strong priors based on the population level fit.

```{r}
# single aggregate fits for single drc
m.stan.single <- standrm_harmony(
  formula=response ~ dose, 
  data=stan_prep_hoescht_dark_avgs_, 
  fct = LL.4(),
  iter = 2000)
...
ed50.stan.single <- ED(m.stan.single, respLev=c(50))
ed50.stan.baseline <- prepare_ed50s_stan(m.stan.all.baseline, stan_prep_hoescht_dark)
ed50.stan.informed <- prepare_ed50s_stan(m.stan.all.informed, stan_prep_hoescht_dark)
```


```{r}
ed50.stan.baseline <- map_do_to_original(ed50.stan.baseline,stan_prep_hoescht_dark)  %>%  
  subset(., select = which(!duplicated(names(.))))
ed50.stan.baseline$group <- "stan_baseline"

ed50.stan.informed <- map_do_to_original(ed50.stan.informed,stan_prep_hoescht_dark)  %>%  
  subset(., select = which(!duplicated(names(.))))
ed50.stan.informed$group <- "stan_informed"

ed50.drc <- map_do_to_original(ed50.drc,stan_prep_hoescht_dark)  %>%    
  subset(., select = which(!duplicated(names(.))))

dual_ed50_df <- rbind(ed50.drc,ed50.stan.baseline, ed50.stan.informed)
...
#plot_drc <- ggplot_build(plot(m.drc.single))
...
ggplot(data = stan_prep_hoescht_dark,
  ...
  geom_line(data = plot_stan$data[[3]], aes(x=x,y=y, col="stan_fit"), size= 3) +
  geom_smooth(method = drm, aes(col = "drc_fit"), method.args = list(fct = LL.4()), se = F) +
...
# ) +
  ...
  ggtitle("Hoescht Dark DRC + Stan fit/ed50 estimation") +
   scale_x_log10() #plots out the dataset with the corresponding 4-parameter log-logit
  #xlim(0,10)
...
long_dual_ed50_df <- base::merge(x = ed50.drc, y = ed50.stan.informed, by = "individual", all = TRUE)
...
agg_w_stan <- merge(x = agg_values, y = ed50.stan.informed, by = "individual", all = TRUE)
...
p_drc_meanreponse <- ggplot() + 
  ...
  ggtitle("mean response vs drc ed50 (drc outliers removed)") +
...
rethinking::precis(fit_growth, pars=c("alpha","beta","lambda", "tau","sigma"))
