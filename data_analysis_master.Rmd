...
all_plates_df_join <- readr::read_csv("lookups/do_unfiltered/growth_lookup.csv") %>% 
  clean_names() %>% 
  tidyr::drop_na(individual) %>% 
  dplyr::select(where(not_any_na)) 

# split pipeline for qa
all_plates_df_join <- all_plates_df_join %>%
  left_join(doscreen_sample_lookup, by = c("individual" = "sample_1")) %>%
  right_join(all_plates_df, by = c("individual")) %>%
  mutate(viable_per = as.numeric(gsub(",","",viable_per)))
...
schema_construction_wells <- all_plates_df_join %>%
  dplyr::count(plate, run, concentration) %>% # may be plate_id
  tidyr::spread(concentration, n, fill = 0) %>%
  arrange(plate) %>%
  mutate(cumsum = `0`+`0.01`+`0.1`+`0.75`+`1`+`1.25`+`2`+`5`) # sum rows by sex and concentration

DT::datatable(schema_construction_wells, rownames=FALSE, caption="Concentration frequencies by 'run' (plate)")
```
