...
- Asserting plate names (i.e. A1, B1, etc) in new column 'plate_name' as seen in found code
- Added F to sex in gender
...
- Loads from ./data/discovery_phase/in_vitro/founders_quantal
...
- Alluvial plots checking samples and comprehensivity 
- Used various Parallel Coordinate Plots to evaluate sample distributions

```{r}
founder_schema <- founders_quantal_all_plates_df %>% 
  dplyr::count(sex, strain, plate_name, concentration) %>% 
  tidyr::spread(concentration, n, fill = 0) %>%
  arrange(desc(strain))
rmarkdown::paged_table(founder_schema, options = {rows.print=10})
```
...
- Loads from ./data/discovery_phase/in_vitro/do_nucleiCounts
...
### DRC boxplot
- tbd handle 0.5 concentration for DO
