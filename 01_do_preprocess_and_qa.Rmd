...
harmony_collection$all_plates <- harmony_collection$all_plates %>% 
  ...
  dose != 0.5, 
  individual != '11_20') %>%
  mutate(plate=ifelse((dir=='d20223_B2_Cal__20200817T11_02_00Measurement_1_e5'), 'A2', plate)) %>%
  ...
  # replace last 3 columns 
  mutate(keep = ifelse((dir=='dC20255_B2__20200926T13_00_37Measurement_2_e6' & column < 10),'keep',
                ifelse((dir=='dC20264_B1__20200928T12_28_55Measurement_1_e5' & column < 9),'keep',
  ...
  # final coat 
  # mutate(group_pair=ifelse((dir=='d20222_B1_Cal__20200825T12_23_59Measurement_3_e6'), 'ZZ', group_pair)) %>%
  # mutate(group_pair=ifelse((dir=='dC20222_B2_rerun__20200829T11_01_59Measurement_4_e6'), 'ZZ', group_pair)) %>%
  filter(keep == 'keep') #%>% 
...
# check_pair <-subset(check,group_pair == 'V')[,c("plate","row","column","dose","group_pair","individual","dir","a","keep")]
# table(check_pair$individual,check_pair$group_pair)
...
for_qa <- harmony_collection$all_plates[,c('dir','row','column','dose','individual','mouse', 'group_pair',custom_feature_selection)] %>%
  filter(!is.na(mouse)) %>%
  rename(plate = dir,
         h2ax = h2axpositive_infocus,
         alexa = infocus_intensity_nucleus_alexa488_mean_mean,
         roundness = infocus_cell_roundness_mean,
         nuclei = nuclei_numberofobjects) %>%
...
for_qa <- for_qa %>% 
  group_by(group_pair) %>% 
  mutate(M = median(nuclei, na.rm =TRUE)) %>%
...
baseline_me_model <- lmer(alexa ~ (1|individual) + (1|plate) + fdose, data=purrr::map_df(for_qa,rev), REML=FALSE)
...
# check inter-mouse effects
pdf(file = "output/baseline_me_model.pdf", height = 80) 
sjPlot::plot_model(baseline_me_model,show.values = FALSE)
dev.off()
```

```{r}
arial_grey_theme <- ggplot2::theme(
...
p <- ggplot(data = for_qa) + theme(legend.position = "none") + geom_boxplot( aes(x = individual, y = nuclei, colour = bin_id), show.legend = FALSE,outlier.size = -1) + ylab("nuclei") + 
    # geom_text(data  = p_meds, aes(x = individual, y = h2ax, label = n), 
    #           size = 2) +
  geom_hline(aes(yintercept=M), colour="#d44846", linetype ='longdash') 

p + facet_wrap(~group_pair, scales = "free_x")  + theme(legend.position = "none") + arial_grey_theme
dev.off()
```
