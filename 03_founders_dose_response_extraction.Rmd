...
prepare_for_drc <- function(data,response_var,weight_var){
    # prepare data
  return( data %>%
    stats::na.omit() %>% 
...
try_mixed_drm_v3 <- function(data, 
  ...
  # prepare data
  data <- prepare_for_drc(data,response_var,weight_var)
...
modify_feature <- function(response_var, data, transform, instructions){
    data <- data %>%
      stats::na.omit() %>% 
      dplyr::mutate(sex = as.factor(sex),
            dose = as.numeric(as.character(dose))) %>%
  ...
  if('remove_outliers' %in% transform){
    # basic outlier removal
    outliers <- boxplot(data$response, plot=FALSE)$out
    r <- with(data, which(response %in% outliers), arr.ind=TRUE)
    data <- data[-r, ]
  }
  if ('zero_replace' %in% transform){
    print("replacing zero with 0.0001")
...
drc_basic <- function(data, formula_selection){
  
   m <- drc::drm(response ~ dose, 
          data = data,
          curveid = strain,
          fct = formula_selection)

  return (m)
}

drc_basic_w_limits <- function(data, 
                      response_var, 
                      fct,
                      strain_name){
   ...
   m <- drm(response ~ dose, 
          data = data,
          curveid = strain,
          lowerl = c(-Inf, 0, min(data$response), 0),
          upperl = c(Inf, min(data$dose), max(data$response), Inf),
          fct = fct)
...
```{r}
    ...
    group_names <- unique(as.character(founders_quantal_collection$all_plates$strain))
    temp_df <- data.frame(matrix(ncol = (length(group_names) + 1), nrow = 0))
    colnames(temp_df) <- c("name", group_names)
```
...
system.time({
    ...
    cluster_results_df <- foreach (f_ = founders_quantal_collection$features_df$name_[1:99], .combine = 'rbind') %:% 
      
      # generate inner cluster around formulas
      # this controls column generation
      # you must pass required libraries inside 
      foreach (fct_ = fcts, .combine = 'cbind', .inorder=TRUE, .packages = c("drc", "dplyr","stats")) %dopar%  {
        tryCatch(

            expr = {
              
              # look up response transformation instructions
              data <- modify_feature(response_var = f_,
                                     data = founders_quantal_collection$all_plates,
                                     transform = c("log_skew_transform"))
                  
              # pick and run a DRC model
             m <- drc::drm(response ~ dose, 
                    data = data,
                    curveid = strain,
                    fct = fct_)
              # model succeeded - will return NA vector as columns
              coefs <- m$coefficients

              temp_df[1,] <- c(f_, tail(coefs, n = length(group_names)))
              temp_df
            },
            error = function(e){

              temp_df[1,] <- c(f_, rep(NA, length(group_names)))
              temp_df
            }
        )
...
```{r}
data2 <- modify_feature(response_var = 'infocus_numberofobjects',
                                     data = founders_quantal_collection$all_plates,
                                     transform = c("log_skew_transform"))
 # m <-drc_basic(founders_quantal_collection$all_plates,
 #    response_var = f_,
 #    fct = fct_
 #  )
               # m2 <-drc_basic(data,
...
m2 <- drm(response ~ dose,
          ...
          fct = L.4(names = c("hill", "min_value", "max_value", "ec_50")))
              ...
              if(!identical(groups,unique(m$parNames[[3]]))){
                stop("insufficient data")
              }
              temp_block <- temp_df
              # append formula name to column name
              colnames(temp_block) <- c("barf",paste("_", unique(m2$parNames[[3]]), sep=""))
              temp_block[1,] <- c(f_, tail(coefs, n = length(unique(m$parNames[[3]]))))
              temp_block
