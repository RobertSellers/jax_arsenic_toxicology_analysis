...
mixed_drc_plot_v2 <- function(x, var) {
  # data prep
  logx = TRUE
  level = NULL
  ranef = TRUE
...
transformations <- c('log_skew_transform','zero_replace', 'scale_positive')

df_instructions <- founders_quantal_collection$features_df

param_fct <- list("LL.4"=LL.4(),"W1.4"=W1.4(),"W2.4"=W2.4())

model_success_list <- vector("list", length(unique(founders_quantal_collection$all_plates$strain)))
names(model_success_list) <- unique(founders_quantal_collection$all_plates$strain)

#for (f in founders_quantal_collection$features_df$name_){
...
example_set <- colnames(founders_quantal_collection$all_plates[,start_idx:end_idx])

# replace metadata columns -- misaligned
...
system.time(
    ...
    for (f_ in example_set){
      for (fct_ in param_fct){
        tryCatch(
          
            expr = {
              print(paste0("checking ",f_, " with ", fct_$name))
              temp_df[which(temp_df$feature == f_, arr.ind=TRUE), fct_$name] <- "fail"
              m <-suppressWarnings(try_mixed_drm_v3(founders_quantal_collection$all_plates,
                response_var = f_,
             ...
             ed50_df[which(ed50_df$feature == f_, arr.ind=TRUE), fct_$name] <- drc::ED(m, c(50), interval="tfls", bound=TRUE)[1]
            ...
            error = function(e){
              ...
              # evaluate success and merge models
              print("-----------")
            }
        )
...
# drc::ED(m, c(50), interval="tfls", bound=TRUE) 
# 
# p <- suppressWarnings(mixed_drc_plot_v2(m, 
#     var = 'something'
# ))
# p
```
