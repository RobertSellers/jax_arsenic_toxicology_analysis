...
feature_subset$response <- scales::rescale(feature_subset$infocus_nucleus_hoechst_ser_dark_1px_mean_m, to = c(0.001, 0.999), from = range(feature_subset$infocus_nucleus_hoechst_ser_dark_1px_mean_m, na.rm = TRUE, finite = TRUE))
...
growth_model <- "
  ...
  parameters {
    ...
    real <lower=-1,upper=1> b; //Hill’s slope of the curve 
  real<lower=0> tau; 
  }
